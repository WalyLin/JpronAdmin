...
class CoreMiddleware implements CoreMiddlewareInterface
{
    ...
    protected function transferToResponse($response, ServerRequestInterface $request) : ResponsePlusInterface
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['response', 'request'], 'keys' => compact(['response', 'request']), 'variadic' => ''], function ($response, ServerRequestInterface $request) use($__function__, $__method__) {
            ...
            if (is_array($response) || $response instanceof Arrayable) {
                return $this->response()->addHeader('content-type', 'application/json')->setBody(new SwooleStream(Json::encode($response)));
            }
            if ($response instanceof Jsonable) {
                return $this->response()->addHeader('content-type', 'application/json')->setBody(new SwooleStream((string) $response));
            }
    ...
    private function getInjections(array $definitions, string $callableName, array $arguments) : array
    {
        $__function__ = __FUNCTION__;
        $__method__ = __METHOD__;
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['definitions', 'callableName', 'arguments'], 'keys' => compact(['definitions', 'callableName', 'arguments']), 'variadic' => ''], function (array $definitions, string $callableName, array $arguments) use($__function__, $__method__) {
            $injections = [];
