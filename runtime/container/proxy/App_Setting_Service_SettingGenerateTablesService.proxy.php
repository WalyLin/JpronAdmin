...
class SettingGenerateTablesService extends AbstractService
{
     ...
     */
    public function __construct(SettingGenerateTablesMapper $mapper, DataMaintainService $dataMaintainService, SettingGenerateColumnsService $settingGenerateColumnsService, ModuleService $moduleService, ContainerInterface $container, private readonly ValidatorFactory $validatorFactory)
    {
    ...
    public function loadTable(array $params) : bool
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['params'], 'keys' => compact(['params']), 'variadic' => ''], function (array $params) use($__function__, $__method__) {
            // 非系统数据源，同步远程库的表结构到本地
            ...
            foreach ($params['names'] as $item) {
                $this->validatorFactory->validate(data: $item, rules: ['name' => 'required|string', 'comment' => 'required|string']);
                $tableInfo = ['table_name' => env('DB_PREFIX') ? str_replace(env('DB_PREFIX'), '', $item['name']) : $item['name'], 'table_comment' => $item['comment'], 'menu_name' => $item['comment'], 'type' => 'single'];
